...
You have musicians names which have the first letter capitalized but are joined:
...
You have a product description where you want to capture the ingredient, which have an specific pattern:

```{r}
description <- c("Bakery: Desdemona (Desserts; A9B-APO) 7th Avenue")
```

All the ingredients have the following structure: three alphanumeric characters, an hyphen and three alphanumeric characters. So we want to capture this pattern:
